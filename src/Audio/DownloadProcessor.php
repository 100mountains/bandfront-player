...
{
    private array $allowedFormats = ['wav', 'mp3', 'flac', 'aiff', 'alac', 'ogg'];

    public function __construct()
    {
        // Constructor can be extended to inject dependencies if needed
    }

    public function validateAudioFormat(string $format): string|false
    {
        return in_array(strtolower($format), $this->allowedFormats) ? strtolower($format) : false;
    }

    public function handleBulkAudioProcessing(): void
    {
        ...
        // Validate format
        $valid_format = $this->validateAudioFormat($format);
        if (!$valid_format) {
