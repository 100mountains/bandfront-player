...
class DbRenderer {
   ...
   private function buildSettingsConfig(array $allSettings): array {
    ...
    // Retrieve all settings from the WordPress options table (actual database)
    $typeMap = [];
    global $wpdb;

    // Get all option names that start with _bfp_ or are enable_db_monitoring
    $option_names = $wpdb->get_col("
        SELECT option_name FROM {$wpdb->options}
        WHERE option_name LIKE '_bfp_%'
        OR option_name = 'enable_db_monitoring'
    ");
    ...
    foreach ($option_names as $option_name) {
        ...
        $typeMap[$option_name] = [
         'type' => $type,
         'default' => null,
         'label' => $this->humanizeKey($option_name)
        ];
    }
