...
class Hooks {
    ...
    private function registerHooks(): void {
        ...
        add_action('woocommerce_before_shop_loop_item_title', [$this, 'maybeAddShopPlayer'], 15);
    ...
    private function registerFrontendHooks(): void {
        ...
        $audio = $this->bootstrap->getComponent('audio');
    ...
    public function registerWidgets(): void {
        ...
        if (class_exists('WooCommerce')) {
            register_widget(PlaylistWidget::class);
        }
    }
    ...
    private function registerWooCommerceHooks(): void {
        ...
        // Enqueue downloads page assets at the right time
        add_action('wp_enqueue_scripts', function() use ($woocommerce) {
            Debug::log("Hooks.php:" . __LINE__ . " wp_enqueue_scripts hook registered for WooCommerce assets");
            // Check if we're on the account page
            Debug::log("Hooks.php:" . __LINE__ . " wp_enqueue_scripts callback executing", ["is_account_page_exists" => function_exists("is_account_page"), "current_url" => $_SERVER["REQUEST_URI"] ?? "unknown"]);
            if (is_account_page()) {
                Debug::log("Hooks.php:" . __LINE__ . " Calling enqueueDownloadsAssets", ["is_account_page" => is_account_page()]);
                $woocommerce->enqueueDownloadsAssets();
            }
        }, 20);
        ...
        // Account Menu hooks - just for menu reordering, no endpoints
        if ($accountMenu = $this->bootstrap->getComponent('account_menu')) {
            $accountMenu->registerHooks();
        }
    }
    ...
    public function maybeAddShopPlayer(): void {
        error_log('[BFP] maybeAddShopPlayer called');
        // Don't continue if not in a shop context
        // Check if we're in a shop context (including when shop is the front page)
        $isShopPage = is_shop() || (is_front_page() && get_option('woocommerce_shop_page_id') == get_option('page_on_front'));
        
        if (!$isShopPage && !is_product_category() && !is_product_tag() && !is_product_taxonomy()) {
            return;
        }
        ...
        if ($player && $playerEnabled && $onCover) {
            ...
            // Get product and check if it has files
            $product = wc_get_product(get_the_ID());
            if (!$product) {
                return;
            }
            
            $fileManager = $this->bootstrap->getComponent('file_manager');
            $files = $fileManager->getProductFilesInternal([
                'product' => $product,
                'first'   => true,
            ]);
            ...
            if (!empty($files)) {
                ...
                // Show navigation buttons only if enabled in global settings
                $globalSettings = get_option('bfp_global_settings', []);
                if (!empty($globalSettings['_bfp_show_navigation_buttons'])) {
        ...
        } elseif ($player && $playerEnabled && !$onCover) {
            // Render regular compact player when onCover is false
            error_log("[BFP] Rendering regular compact player for product: " . get_the_ID());
            $output = $player->renderCompact(get_the_ID());
            error_log("[BFP] renderCompact output length: " . strlen($output));
            echo $output;
        }
    }
