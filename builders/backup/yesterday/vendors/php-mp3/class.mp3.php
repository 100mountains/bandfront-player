...
class BFPMP3 {
    ...
    // Updated with official ID3v1 genres (corrected spellings and removed questionable entries)
    public $id3v1_genres = [
        'Blues',
        'Classic Rock',
        'Country',
    ...
    public $id3v2_frame_descriptions = [
    ...
    private function validate_mp3_file($filepath) {
    ...
    function get_mp3($filepath, $analysis = false, $getframesindex = false) {
        ...
        // Add input validation
        if (!$this->validate_mp3_file($filepath)) {
            return false;
        }
        ...
        $this->id3v1 = $this->id3v2 = $this->data = [];
    ...
    private function get_id3v2() {
        ...
        // Add null check for file pointer
        if (!$this->fp) {
            return false;
        }
        ...
        if (!$tagheaderdata || strlen($tagheaderdata) < 10) {
        ...
        while (1) {
            ...
            if (!$frameheaderdata || strlen($frameheaderdata) < 10) {
                continue;
            }
    ...
    private function get_id3v1() {
        ...
        if (!$tagdata || strlen($tagdata) < $tagsize) {
            return false;
        }
        ...
        if (
            !empty($tag['genre']) &&
            isset($this->id3v1_genres[$tag['genre']])
        ) {
    ...
    private function get_data_frames() {
        ...
        while (1) {
            ...
            if ($this->fileanalysis > 1) {
                $this->audio_frames[] = [$pos_frame - 4, $bitrate, $framesize];
            }
    ...
    function set_mp3($file_input, $file_output, $id3v2 = [], $id3v1 = []) {
        ...
        $id3v2 = is_array($id3v2) ? $id3v2 : [];
        $id3v1 = is_array($id3v1) ? $id3v1 : [];
    ...
    function cut_mp3($file_input, $file_output, $startindex = 0, $endindex = -1, $indextype = 'frame', $cleantags = false) {
        ...
        if (!in_array($indextype, ['frame', 'second', 'percent'])) {
            return false;
        }
