<?php
if ( ! class_exists( 'WCMP_CLOUD_DRIVE' ) ) {
	class WCMP_CLOUD_DRIVE {
		...
		public function init() {
			...
			if ( $this->connect() ) {
				try {
					if ( ( $folder_id = $this->exists( 'wcmp', 'application/vnd.google-apps.folder' ) ) == 0 ) {
						$folder_id = $this->createFolder( 'wcmp' );
					}
		...
		public function connect() {
			try {
				...
				if ( file_exists( $this->_credential_file ) && ! isset( $_GET['wcmp-drive-credential'] ) ) {
			...
			if ( ( $folder_id = $this->init() ) != false ) {
				try {
					...
					if ( $file ) {
						...
						return array(
							'id'  => $file_id,
							'url' => 'https://www.googleapis.com/drive/v3/files/' . urlencode( $file_id ) . '?alt=media&key=' . urlencode( get_option( '_wcmp_drive_api_key', '' ) ) . '&ext=.' . ( ( $extension ) ? $extension : 'mp3' ),
						);
					}
				} catch ( Exception $e ) {
					$this->_errors[] = $e->getMessage();
